...
# @Time : 2023/12/22 12:07
# @Author : 娄星华
# @File : lora_fine_tuning
# @Project : SAM
...
if __name__ == "__main__":
    ...
    image_dirPath = lib.image_dirPath
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for k in keys[:20]:
            ...
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            transform = ResizeLongestSide(lora_sam.sam.image_encoder.img_size)
            input_image = transform.apply_image(image)
            input_image_torch = torch.as_tensor(input_image).to(device=device)
            transformed_image = input_image_torch.permute(2, 0, 1).contiguous()[None, :, :, :]

            input_image = lora_sam.sam.preprocess(transformed_image).to(device=device)
            original_image_size = image.shape[:2]
            input_size = tuple(transformed_image.shape[-2:])
            ...
            # # 手动计算并记录梯度范数
            # total_norm = 0
            # for p in lora_sam.sam.parameters():
            ...
            # # 添加梯度裁剪
            # import torch.nn.utils as utils
            # utils.clip_grad_norm_(lora_sam.sam.parameters(), max_norm=1)  # 可以根据需要调整
            #
            # # 手动计算并记录梯度范数
            #
            # total_norm = 0
